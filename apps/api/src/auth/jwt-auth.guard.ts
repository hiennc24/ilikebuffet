/**
 * Global JWT authentication guard (Red Team H2).
 *
 ...
 *  4. Token-version check via RevocationService (Redis + DB fallback) — M4/V2.
 *     Locking / logout-all bumps DB tv AND invalidates Redis → ≤30s revocation.
 *  5. mustChangePassword gate (C2): if mcp=true in token AND route is not
...
export class JwtAuthGuard extends AuthGuard("jwt") {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Per-request revocation check (M4/V2): compare JWT tv with Redis/DB tv.
    ...
    // mustChangePassword gate (C2): block all routes except @PasswordChangeAllowed.
    if (payload.mcp) {
